...
measure_timer::measure_timer ( QObject *parent ) : QObject ( parent ) {//measure的构造函数，定时5秒发送请求数据包
    timer = new QTimer();
    timer->setInterval ( 1000 * 10 ); /* 设置定时时间为5秒钟 */
...
void measure_timer::getMeasure ( void ) {//发送请求数据包
    ...
    qDebug() << tr ( "测量中..." );
        ...
        address.setAddress ("2001:250:500a:111:7d8b:2cf9:bee3:60b" );
        //address.setAddress ( "192.168.1.115" );
        qDebug()<<messageid;
       if(udpSocket->writeDatagram ( ( const char * ) ( pdu->getPDUPointer() ), pdu->getPDULength(), address, 5680 )>0)
           qDebug()<<tr("发送测量成功");
       else
           qDebug()<<tr("发送测量失败");
}

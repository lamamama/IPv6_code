...
function sech(id) {//省市改变时触发，select的onchange事件
    ...
    if(id=="xian") {
        getRoad(aa.value);//这里aa.value为县的id
    }
    if(id=="lu") {
        getLamp(aa.value);//这里aa.value为路的id
    }
...
function getRoad(id){//获取市对应的区
    ...
    var url= "getDetails.php?ID=" + escape(id);
    request.open("GET", url, true);
    request.onreadystatechange = displayRoad;
    request.send(null);
}



function getLamp(id){//获取市对应的区
    request = createRequest();
    if (request == null) {
        alert("Unable to create request");
        return;
    ...
    var url= "getDetails.php?ID=" + escape(id);
    // var url= "getDetails.php?ID=" + 1010101;
    request.open("GET", url, true);
    request.onreadystatechange = displayLamp;
    request.send(null);
}
  ...
  if (request.readyState == 4) {
      if (request.status == 200) {
        ...
        if(document.getElementById("sheng").value!="province"&&document.getElementById("shi").value!="city"){
            ...
            for(i=0;i<a.length;i++)
                obj.options.add(new Option(a[i],document.getElementById("shi").value*100+i+1));
        }
...
function displayRoad() {//将获取的数据增加至select
    if (request.readyState == 4) {
        if (request.status == 200) {
            var a=new Array;
            var b=request.responseText;
            a=b.split(",");
            document.getElementById("lu").length=1;
            if(document.getElementById("sheng").value!="province"&&document.getElementById("shi").value!="city"&&document.getElementById("xian").value!="county"){
                var obj=document.getElementById('lu');
                for(i=0;i<a.length;i++)
                    obj.options.add(new Option(a[i],document.getElementById("xian").value*100+i+1));
            }

        }
    }
}
function displayLamp() {//将获取的数据增加至select
    if (request.readyState == 4) {
        if (request.status == 200) {
            var a=new Array;
            var b=request.responseText;
            // var c="1010101001,";
            a=b.split(",");
            document.getElementById("deng").length=1;
            if(document.getElementById("sheng").value!="province"&&document.getElementById("shi").value!="city"&&document.getElementById("xian").value!="county"&&document.getElementById("lu").value!="road"){
                var obj=document.getElementById('deng');
                for(i=0;i<a.length;i++)
                    obj.options.add(new Option(a[i],document.getElementById("lu").value*1000+i+1));
            }
        }
    }
}
/*
1. 声明一个进程 coap_process
2. 定义一个结构体变量 static service_callback_t service_cbk = NULL;
3. 函数coap_receive(void)  //		
4. coap_init_engine(void) // process_start(&coap_engine, NULL);
5. 进程coap_engine的定义  功能：将res_well_known_core资源添加到list并指定uri，然后创建udp，监听默认窗口
...
extern void uart1_send_string ( char *buf );
...
PROCESS_THREAD(coap_engine, ev, data)
{
  ...
  while(1) {
    PROCESS_YIELD();
    uart1_send_string("----I get somthing------\r\n");
